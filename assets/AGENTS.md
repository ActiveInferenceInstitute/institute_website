...
| `redirects.js` | Legacy Squarespace-URL → clean-URL map. Loaded **only** on `404.html` (GitHub Pages serves it for unresolved paths). Reads `data-base` and calls `location.replace()`; no network. See [`../docs/MIGRATION_REDIRECTS.md`](../docs/MIGRATION_REDIRECTS.md). |
