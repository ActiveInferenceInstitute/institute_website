...
| `redirects.js` | Two mechanisms: `MAP` (legacy, English-only Squarespace-URL → clean-URL aliases) and `PREFIX_REDIRECTS` (locale-aware slug-prefix → new output-dir renames from URL-taxonomy changes, applied across all 12 locales). Loaded **only** on `404.html` (GitHub Pages serves it for unresolved paths). Reads `data-base` and calls `location.replace()`; no network. See [`../docs/MIGRATION_AND_REDIRECTS.md`](../docs/MIGRATION_AND_REDIRECTS.md). |
